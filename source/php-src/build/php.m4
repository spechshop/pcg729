...
AC_DEFUN([PHP_SETUP_ICU],[
  PKG_CHECK_MODULES_STATIC([ICU], [icu-uc >= 57.1 icu-io icu-i18n])
...
AC_DEFUN([PHP_SETUP_OPENSSL],[
  ...
  PKG_CHECK_MODULES_STATIC([OPENSSL], [openssl >= 1.1.1], [found_openssl=yes])
...
AC_DEFUN([PHP_SETUP_LIBXML], [
  PKG_CHECK_MODULES_STATIC([LIBXML], [libxml-2.0 >= 2.9.4])
...
PKG_CHECK_MODULES_STATIC([SQLITE], [sqlite3 >= 3.7.17], [
...
PKG_CHECK_MODULES_STATIC([ZLIB], [zlib >= 1.2.11], [dnl
...
AS_VAR_IF([pgsql_dir],,
  [PKG_CHECK_MODULES_STATIC([PGSQL], [libpq >= 10.0],
    [found_pgsql=yes],
    [found_pgsql=no])])
